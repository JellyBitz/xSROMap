...
var xSROMap = function(){
	...
	var flyView = function (coord){
		...
		// link shortcut
		const copyLink = '<a class="leaflet-popup-copy-button" title="Copy Link" href="#" onClick="xSROMap.LinkToClipboard('+coord.x+','+coord.y+','+coord.z+','+coord.region+')"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 576" style="vertical-align:middle"><path d="M320 448v40c0 13.255-10.745 24-24 24H24c-13.255 0-24-10.745-24-24V120c0-13.255 10.745-24 24-24h72v296c0 30.879 25.121 56 56 56h168zm0-344V0H152c-13.255 0-24 10.745-24 24v368c0 13.255 10.745 24 24 24h272c13.255 0 24-10.745 24-24V128H344c-13.2 0-24-10.8-24-24zm120.971-31.029L375.029 7.029A24 24 0 0 0 358.059 0H352v96h96v-6.059a24 24 0 0 0-7.029-16.97z"/></svg></a>';
		const mapCoord = CoordSROToMap(coord);

		// show popup
		L.popup().setLatLng(mapCoord).setContent(copyLink+content).openOn(map);

		map.flyTo(mapCoord,8,{duration:2.5});
	};
	...
	var fixCoords = function(x,y,z,region) {
		...
		var xSector = region & 0xFF
		var ySector = region >> 8;
		var posX = (xSector - 135) * 192 + x / 10;
		var posY = (ySector - 92) * 192 + y / 10;

		// using x,y,z,region internal silkroad coords
		return {'x':x,'y':y,'z':z,'region':region, 'posX': posX, 'posY': posY};
	};
